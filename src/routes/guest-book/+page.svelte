...
<section class="flex-1 grow space-y-1 overflow-y-auto overscroll-y-contain px-3 lg:px-4">
	{#if data}
		<form class="flex flex-col gap-2 text-sm lg:flex-row lg:items-center" {...insertGuestBook}>
			...
			/>
			{#if data.user}
				<button onclick={() => trigger()} class="bg-ash-400 text-ash-800 flex w-full items-center justify-center gap-2 gap-x-2 px-2 py-0.5 lg:w-40">
					Submit
				</button>
			{:else}
				<a onclick={() => trigger()} class="bg-ash-400 text-ash-800 flex w-full items-center justify-center gap-2 gap-x-2 px-2 py-0.5 lg:w-40" href="/api/auth">
			...
			{#each data.guestBooks as item (item.id)}
				...
				<li class="group flex flex-col gap-1 py-1 lg:flex-row lg:gap-2 lg:border-y-0 lg:py-0" class:animate-pulse={item.id < 0}>
					<div class="flex items-center justify-between gap-2 lg:contents">
						<p class="min-w-0 flex-1 truncate lg:w-36 lg:flex-none">
							<span class="text-cyan">~</span>/{item.username.toLowerCase().replace(/\s/g, '-')}
						</p>
						{#if data.user && item.id > 0}
							<div class="flex shrink-0 items-center gap-x-1 lg:hidden">
								{@render actions()}
							</div>
						{/if}
					</div>
					...
					{#if data.user && item.id > 0}
						<div class="hidden shrink-0 items-center gap-x-1 lg:flex">
							{@render actions()}
						</div>
					{/if}
