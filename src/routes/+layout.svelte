<script lang="ts"> import { MediaQuery } from 'svelte/reactivity'; import { PUBLIC_UMAMI_TRACKING_ID } from '$env/static/public'; import { page } from '$app/state'; import '../app.css'; import { dev } from '$app/environment'; import Header from '$lib/components/layout/header/header.svelte'; import Navbar from '$lib/components/layout/navbar/navbar.svelte'; let { data, children } = $props(); let dragging = $state(false); let isFullscreen = $state(false); let offset = $state({ x: 0, y: 0 }); let position = $state({ x: 0, y: 0 }); let containerElement = $state<HTMLElement | null>(null); const mobileQuery = new MediaQuery('(max-width: 1024px)'); let isMobile = $derived(mobileQuery.current); let isLegalPage = $derived(page.url.pathname.startsWith('/legal')); function toggleFullscreen() { if (isMobile) return; if (!isFullscreen) { containerElement?.requestFullscreen(); } else { document.exitFullscreen(); position = { x: 0, y: 0 }; } isFullscreen = !isFullscreen; } function onMouseDown(e: MouseEvent) { if (isMobile) return; dragging = true; offset = { x: e.clientX - position.x, y: e.clientY - position.y }; } function handleMouseUp() { dragging = false; } function handleMouseMove(e: MouseEvent) { if (dragging) { position = { x: e.clientX - offset.x, y: e.clientY - offset.y }; } } function handleFullscreenChange() { if (!document.fullscreenElement) { isFullscreen = false; } } $effect(() => { if (!isMobile) return; position = { x: 0, y: 0 }; }); </script>
...
{#if !dev}
	<script
		...
		data-website-id={PUBLIC_UMAMI_TRACKING_ID}
	></script>
{/if}
...
<div class="content" class:legal-scroll={isLegalPage}>
	{@render children()}
</div>
...
<style> .content { min-height: 0; } .legal-scroll { flex: 1; min-height: 0; overflow-y: auto; overflow-x: hidden; } </style>
